...
We think of this type of approach as a form of canary detection, rather than strategies based on blocking content based requests.
The signal that a potential attackers gives us by sending requests to a known-bad path allow us to block the attacker source for a
predetermined length of time - and potentially extend the block if the attack continues to attempt to send us traffic.

Benefits of this type of approach include reducing noise as far upstream as possible (i.e. at the edge), leveraging bad traffic
directed at one application or API to protect other applications and APIs on our domain. Even though we don't expect to block all, or
anywhere near all bad traffic, we can make harder for bad actors by blocking or slowing down their activity.

## How it works

The architecture for a proof-of-concept is fairly simple. We'll deploy a small code snippet to a Cloudflare Worker to inspect requests
and write IP blocks to Cloudflare KV with a preconfigured TTL to auto-expire the blocks. We'll also extend the block if an attacker continues
...
- Read-only - just log, don’t block
- Normal - 1 bad path request = IP block, expires in 1 hour
- Paranoid - 1 bad path request = IP block, every subsequent path hit extends block by 3 hours
